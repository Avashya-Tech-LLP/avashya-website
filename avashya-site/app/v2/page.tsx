...
function NeuralCanvas() {
  ...
  useEffect(() => {
    ...
    const onMouseUp = () => {
      if (isDragging && draggedNode) {
        // Give the node a little velocity based on recent movement
    ...
    const draw = () => {
      ...
      for (const node of nodes) {
        ...
        // Attract toward mouse when hovering nearby
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl border-b" style={{ background: 'var(--color-nav-bg)', borderColor: 'var(--color-border)' }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <a href="/v2/join-us" className="text-sm font-medium transition-colors hover:text-[var(--color-primary)]" style={{ color: 'var(--color-text-secondary)' }}>Join Us</a>
        </div>
...
function OverviewVideoSection() {
  return (
    <section className="relative py-16 sm:py-20" style={{ background: 'var(--color-cream)' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <VideoPlaceholder title="Watch: How Avashya transforms engineering orgs — 2 min overview" />
        </motion.div>
      </div>
    </section>
  );
